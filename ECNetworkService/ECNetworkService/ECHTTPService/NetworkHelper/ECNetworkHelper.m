...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    //读取缓存
    responseCache!=nil ? responseCache([ECNetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
    
    //判断是否正在进行上一个网络请求
    if ([self allSessionTask].count) {
        NSArray* allTaskArr = [[self allSessionTask] copy];
        for (NSURLSessionTask  *task in allTaskArr) {
            if ([task.currentRequest.URL.absoluteString hasPrefix:URL] && task.state == NSURLSessionTaskStateRunning) {
                NSLog(@"重复请求%@",URL);
                return nil;
            }
        }
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        //对数据进行异步缓存
        responseCache!=nil ? [ECNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
